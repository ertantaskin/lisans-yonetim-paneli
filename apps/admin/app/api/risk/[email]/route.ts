 ...
 * isteği apiRaw ile sunucu-taraflı iletir (token + trace-id §16 merkezî) — token tarayıcıya
 * ASLA sızmaz. Erişim, uygulamanın geri kalanıyla aynı oturum gate'i (middleware) altındadır.
 * YALNIZ GÖRÜNTÜ: skor + faktör kırılımı döner, hiçbir otomatik askı/blok tetiklemez. API
 * erişilemezse "duruk" (score/band = null) döner — rozet kibarca "risk verisi yok" gösterir (graceful).
 */
...
export async function GET(_req: Request, { params }: { params: Promise<{ email: string }> }) {
  ...
  try {
    const res = await apiRaw('GET', `/v1/admin/customers/${encodeURIComponent(email)}/risk`, {
