...
class Wpteslimat_Report_Issue {
    ...
    public static function render_button($order, $assignment_id = '') {
        ...
        <details class="wpteslimat-report" style="margin-top:8px">
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="margin-top:6px">
                ...
                <button type="submit" class="button button-small" style="margin-top:6px"><?php echo esc_html__('Gönder', 'wpteslimat'); ?></button>
            </form>
        </details>
        <?php
    }
    ...
    public static function render_notice() {
        if (!isset($_GET['wpteslimat_report'])) return;
        $r = sanitize_key(wp_unslash($_GET['wpteslimat_report']));
        if ($r === 'ok') {
            echo '<div class="woocommerce-message" role="alert">' .
                esc_html__('Talebiniz alındı. Destek ekibimiz en kısa sürede inceleyecek.', 'wpteslimat') .
                '</div>';
        } elseif ($r === 'short') {
    ...
    private static function redirect_back($url, $flag) {
        wp_safe_redirect(add_query_arg('wpteslimat_report', $flag, $url));
        exit;
    }
}
