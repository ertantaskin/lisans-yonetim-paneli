...
class Wpteslimat_My_Account {
    ...
    public function render($order) {
        ...
        // dediyse (teslimat durumu ne olursa olsun) bayat _wpteslimat_held_for_review işaretini kalıcı
        ...
        if (empty($deliveries)) {
            ...
            // (fulfilled/partial/revoked) inceleme sonuçlanmıştır → bayat _wpteslimat_held_for_review
            ...
            foreach ($deliveries as $i => $d) {
                ...
                if ($is_account && !empty($d['fields']) && is_array($d['fields'])) {
                    ...
                    foreach ($d['fields'] as $fi => $f) {
                        ...
                        echo '<button type="button" onclick="navigator.clipboard.writeText(document.getElementById(\'' . esc_js($fid) . '\').textContent)" class="button button-small" style="margin-left:6px">' . esc_html__('Kopyala', 'wpteslimat') . '</button>';
                        echo '</div>';
                    }
                    echo '</div>';
                } elseif ($is_account) {
                    ...
                    echo '<button type="button" onclick="navigator.clipboard.writeText(document.getElementById(\'' . esc_js($id) . '\').textContent)" class="button" style="margin-left:8px">' . esc_html__('Kopyala', 'wpteslimat') . '</button>';
                }
