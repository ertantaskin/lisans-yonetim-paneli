...
products(id, sku, name, kind['key'|'account'|'custom'|'code'], usage_mode['single'|'multi'], max_uses, multi_use_distribution['fewest-keys'|'one-per-key'], validity_days, on_expiry['hide'|'keep'], stockless, release_at, fulfillment_policy, warranty_days, low_stock_threshold, created_at, updated_at)
